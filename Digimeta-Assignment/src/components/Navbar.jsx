...
import { AiOutlineQuestionCircle, AiOutlineSearch, AiOutlineBell,  } from 'react-icons/ai'
  ...
  return (
    <div className='w-full fixed top-0 z-30 h-12 bg-[#001529] flex justify-between items-center'>
        <span className='flex ml-4'>
            ...
            <p className='text-white ml-3 text-lg font-bold'>Ant Design Pro</p>
        </span>
        <span className='flex items-center'>    
            {
                search ?
                <span className={`mr-8 flex items-center transition duration-300 ease-in-out`} >
                    <AiOutlineSearch className='text-white mr-2' onClick={() => setSearch(false)} />
                    <input type="text" className='w-48 h-6 rounded-sm' />
                </span>
                : 
                <AiOutlineSearch className='text-white mr-8' onClick={() => setSearch(true)} />
            }
